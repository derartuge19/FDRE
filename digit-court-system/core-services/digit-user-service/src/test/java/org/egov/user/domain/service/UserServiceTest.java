...
import org.egov.user.persistence.repository.CourtUserRepository;
...
public class UserServiceTest {
    ...
    public void test_should_search_for_users() {
        ...
        when(encryptionDecryptionUtil.encryptObject(userSearch, "User", UserSearchCriteria.class))
                .thenReturn(userSearch);
        when(encryptionDecryptionUtil.decryptObject(expectedListOfUsers, null, User.class, getValidRequestInfo()))
                .thenReturn(expectedListOfUsers);
    ...
    public void test_should_not_create_citizenWithWrongUserName() {
        userService = new UserService(userRepository, otpRepository, fileRepository, passwordEncoder,
                ...
                true, false, pwdRegex, pwdMaxLength, pwdMinLength, courtUserRepository);
    ...
    public void test_should_set_pre_defined_expiry_on_creating_citizen() {
        ...
        when(encryptionDecryptionUtil.encryptObject(domainUser, "User", User.class)).thenReturn(domainUser);
        when(encryptionDecryptionUtil.decryptObject(expectedUser, "User", User.class, getValidRequestInfo()))
                .thenReturn(expectedUser);
        userService.createCitizen(domainUser, getValidRequestInfo());
    ...
    public void test_should_update_a_valid_user() {
        ...
        when(userService.getUniqueUser(anyString(), anyString(), any(UserType.class))).thenReturn(expectedUser);
    ...
    public void test_should_validate_user_on_update() {
        ...
        User user = User.builder().build();
        Mockito.doNothing().when(userRepository).update(any(org.egov.user.domain.model.User.class),
                any(org.egov.user.domain.model.User.class), any(Long.class), any(String.class));
        when(userRepository.findAll(any(UserSearchCriteria.class))).thenReturn(Collections.singletonList(user));
    ...
    // @Ignore
    //// @Test(expected = DuplicateUserNameException.class)
    //// public void test_should_throw_error_when_username_exists_while_updating()
    // throws Exception {
    //// User domainUser = validDomainUser(false);
    //// when(userRepository.isUserPresent(any(String.class),
    ...
    public void test_should_throwexception_incaseofloginotpenabledastrue_forcitizen_update_password_request() {
        userService = new UserService(userRepository, otpRepository, fileRepository, passwordEncoder,
                encryptionDecryptionUtil, tokenStore, DEFAULT_PASSWORD_EXPIRY_IN_DAYS,
                true, isEmployeeLoginOtpBased, pwdRegex, pwdMaxLength, pwdMinLength, courtUserRepository);
        User user = User.builder().username("xyz").tenantId("default").type(UserType.CITIZEN).build();
        when(userRepository.findAll(any(UserSearchCriteria.class))).thenReturn(Collections.singletonList(user));
        final LoggedInUserUpdatePasswordRequest updatePasswordRequest = LoggedInUserUpdatePasswordRequest.builder()
                .userName("xyz")
    ...
    public void test_should_throwexception_incaseofloginotpenabledastrue_foremployee_update_password_request() {
        userService = new UserService(userRepository, otpRepository, fileRepository, passwordEncoder,
                encryptionDecryptionUtil, tokenStore, DEFAULT_PASSWORD_EXPIRY_IN_DAYS,
                false, true, pwdRegex, pwdMaxLength, pwdMinLength, courtUserRepository);
    ...
    public void test_should_throw_exception_when_existing_password_does_not_match_on_attempting_to_update_user() {
        final LoggedInUserUpdatePasswordRequest updatePasswordRequest = LoggedInUserUpdatePasswordRequest.builder()
                ...
                .existingPassword("existingPassword")
                .build();
        User user = User.builder().username("xyz").tenantId("default").type(UserType.CITIZEN)
                .password("existingPasswordEncoded").build();
    ...
    public void test_should_update_password_for_logged_in_user() {
        final LoggedInUserUpdatePasswordRequest updatePasswordRequest = LoggedInUserUpdatePasswordRequest.builder()
                ...
                .build();
        Long id = (long) 123;
        User domainUser = User.builder().id(id).username("xyz").tenantId("default").type(UserType.CITIZEN)
                .password("existingPasswordEncoded").build();
    ...
    public void test_should_validate_request_when_updating_password_for_non_logged_in_user() {
        ...
        when(userRepository.findAll(any(UserSearchCriteria.class))).thenReturn(Collections.singletonList(domainUser));
        when(encryptionDecryptionUtil.encryptObject(domainUser, "User", User.class)).thenReturn(domainUser);
        when(encryptionDecryptionUtil.decryptObject(domainUser, "User", User.class, getValidRequestInfo()))
    ...
    private RequestInfo getValidRequestInfo() {
        List<org.egov.common.contract.request.Role> roles = Collections
                .singletonList(org.egov.common.contract.request.Role.builder().code("roleCode1").build());
        org.egov.common.contract.request.User userInfo = org.egov.common.contract.request.User.builder().roles(roles)
                .id((long) 123).build();
