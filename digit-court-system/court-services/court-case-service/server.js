...
const PORT = process.env.PORT || 3002; // Standard Microservice Instance
...
// Institutional Case Registry - Refactored to fetch real data
app.get('/api/cases', authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, message: 'Case registry fetch failed' });
  }
});
...
app.post('/api/cases', authenticateToken, async (req, res) => {
  try {
    ...
    const newCase = await hearingDatabase.createCase({
      ...caseData,
    ...
    res.json({
      success: true,
      message: 'Institutional docket committed to database',
      data: newCase
    });
  } catch (error) {
    console.error('Failed to create case in database:', error);
    res.status(500).json({ success: false, message: 'Docket initialization failed' });
  }
});

// Hearings - Refactored to fetch real data
app.get('/api/hearings', authenticateToken, async (req, res) => {
  try {
    const result = await hearingDatabase.query('SELECT * FROM hearings h LEFT JOIN cases c ON h.case_id = c.id');
    res.json({
      success: true,
      data: result.rows,
    ...
    console.error('Failed to fetch hearings from database:', error);
    res.status(500).json({ success: false, message: 'Hearing schedule fetch failed' });
  }
});
